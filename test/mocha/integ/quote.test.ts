...
describe('quote', function () {
  ...
  const executeSwap = async (
    ...
    await getBalanceAndApprove(alice, SWAP_ROUTER_02_ADDRESSES(), currencyIn)
      ...
      describe(`${ID_TO_NETWORK_NAME(1)} ${algorithm} ${type} 2xx`, () => {
        describe(`+ Execute Swap`, () => {
          ...
          it(`erc20 -> erc20 swaprouter02`, async () => {
            ...
            expect(methodParameters).to.not.be.undefined
            expect(methodParameters?.to).to.equal(SWAP_ROUTER_02_ADDRESSES())
          ...
          it(`eth -> erc20 swaprouter02`, async () => {
            ...
            expect(status).to.equal(200)
            expect(data.methodParameters).to.not.be.undefined
            expect(data.methodParameters?.to).to.equal(SWAP_ROUTER_02_ADDRESSES())
  ...
  const TEST_ERC20_2: { [chainId in ChainId]: () => Token | null } = {
    ...
    [ChainId.MODE]: () => DAI_ON(ChainId.MODE),
  }
